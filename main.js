...
const config = {
  ...
  VELOCITY_DISSIPATION: 0.82, // 水の粘り(大きいほどすぐ止まる)
  PRESSURE: 0.8,
  ...
  SPLAT_FORCE: 900,          // ドラッグで水を押す力
  INK_FLOW: 0.18,            // ドラッグ中に出る墨の濃さ
  DROP_INK: 0.44,            // 一滴の濃さ
...
function drop(x, y, color, scale = 1) {
  ...
  const off = 0.0035;
  const force = (6 + Math.random() * 5) * scale;
  const ox = Math.cos(angle) * off;
  const oy = Math.sin(angle) * off;
  const r = config.DROP_RADIUS * 0.9;
